...
export default async function SanteSportPage() {
  const [sanRes, spoRes] = await Promise.all([
    getArticles({ category: 'sante' }),
    getArticles({ category: 'sport' }),
  ]);

  const allArticles = [...sanRes.data, ...spoRes.data].sort((a, b) => 
  ...
  return (
    <div className="category-page container">
      <header className="category-page__header">
        <h1 className="section-title">Santé & Sport</h1>
        <p className="category-page__description">
          Tout pour votre bien-être : actualités médicales, conseils santé et l'essentiel du sport.
        </p>
      </header>
